...
int main() {
	...
	while (1) {
		printf("\nEnter a string (or input 'exit'): ");
		gets_s(input_string, sizeof(input_string)); // get_s 사용해 공백까지 입력받는다.
		if (check_matching(input_string) == 1) printf("%s is a palindrome", input_string); // 참이 리턴되면 실행
		else printf("%s is not palindrome", input_string);
	}
}
